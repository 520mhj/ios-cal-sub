...
| Secret | `CAL_EDITOR_KEY` | `pnpm cal:key` 生成的 UUID 原样(解锁编辑器;兼作私密订阅令牌的派生源) | 用在线编辑器/私密订阅则必填 |
| Variable | `CAL_EDITOR_HINT` | 锁屏提示语 | 可选 |

### 🔐 订阅方式:公开 vs 私密(按日历独立设置)

在编辑页每个日历的「订阅方式」里切换:

- **🌐 公开**(默认):订阅页公开展示该日历的订阅按钮和地址;
- **🔒 私密**:订阅页只显示 🔒 徽标、不出现任何链接;`.ics` 被写入
  不可猜测的 `/s/<令牌>/日历名.ics` 路径,令牌 =
  `sha256( 访问密钥的SHA-256 + '|' + 日历ID )` 前 32 位——
  **每个日历独立派生,泄露一个链接不影响其他日历**。
- 私密日历的专属订阅链接在**编辑页对应日历的区域**查看和复制
  (发给家人即订;iPhone 粘贴到 设置→日历→添加订阅日历)。
- 前提:私密依赖访问密钥,未配置 `CAL_EDITOR_KEY` 时构建会明确报错提示。
- 轮换密钥会使所有私密链接失效——重新生成后到编辑页复制新链接,
  iPhone 删除旧订阅重新添加即可。
