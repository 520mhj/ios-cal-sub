...
| 元宵、七夕、寒衣等**传统节日** Apple 日历根本没有 | `lunar-festival` 源:内置 17 个节日(春节/清明/端午/中秋全在内,含冬至🥟、南北小年);**按农历/节气计算,不依赖国务院公告**,公告未发布的年份照样显示 |
...
打开订阅页 `https://你的用户名.github.io/仓库名/`,任选其一:

- **扫一扫(推荐给家人朋友)**:点日历卡片上的 **「🔳 扫码订阅」**,对方 iPhone 相机对准二维码 → 点链接 → 自动弹出「订阅日历」,一键添加;安卓/桌面用任意扫码应用同样可订;
...
calendars:
  - id: cn-holidays          # 输出文件名(id 只能小写字母数字连字符)
    ...
    sources:
      ...
      - type: lunar-festival  # 农历传统节日(内置 17 个,按农历/节气计算,零外部数据)
