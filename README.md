...
| 元宵、七夕、寒衣等**传统节日** Apple 日历根本没有 | `lunar-festival` 源:内置 14 个节日(含冬至🥟、南北小年,除夕自动落腊月末) |
...
calendars:
  - id: cn-holidays          # 输出文件名(id 只能小写字母数字连字符)
    ...
    sources:
      ...
      - type: lunar-festival  # 农历传统节日(内置 14 个,零外部数据)
        festival: 七夕节       # 元宵节/龙抬头/上巳节/七夕节/中元节/重阳节/寒衣节/下元节
                               # /腊八节/冬至🥟/小年(北方)/小年(南方)/除夕
