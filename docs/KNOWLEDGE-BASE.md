...
| 数据源 | 方式 | 原因 |
|---|---|---|
| 节气连续场景(days>1) | 每年一条 `RRULE:FREQ=DAILY;COUNT=N` | 用户语义即"每年节气日自动发生,往后 N 天";天文日期逐年不同无法 BYMONTHDAY,但锚定 DTSTART 后 DAILY COUNT 完美表达 |
| rule 周/月/年循环 | 单条 `FREQ=...;UNTIL=...` | 天然机械重复;UNTIL 类型必须与 DTSTART 一致(定时→UTC 日期时间,全天→日期) |
...
- **代价要知情**:RRULE 化后 SUMMARY 无法逐日变化——节气打卡的 `(n/N)` 进度后缀随之消失;VALARM 写在 VEVENT 内、对**所有**发生日生效(本场景恰好正确)。
- **UID 策略**:RRULE 定义含年份(`term-春分-0|2026`),物化的含日期;重新生成时旧 UID 消失、新 UID 出现,iPhone 全量替换订阅,不残留重复。
- **教训**:先想清"订阅端如何理解",再决定"文件怎么写"。定义数从 ~590 条降到 ~50 条,体积与语义双收益。
